...
2. Make your change. Match the app's existing patterns rather than
   ...
   [README.md](README.md) for where things live, and pick the file that's
   already closest to what you're doing.
...
All four have to pass clean. `cargo clippy` in particular is run with
...
- **Rust git operations**: every command goes through `run_git`/
  ...
  array (`&["checkout", "-b", &name]`), never a concatenated shell string.
  Network-touching commands (clone/pull/push/ls-remote) use
...
- **Rust tests**: use the `TestRepo` helper in
  `src-tauri/src/git/tests/commands_tests.rs` and call the `_sync` function
  directly, not the async `#[tauri::command]` wrapper. Tests run real `git`
  subprocesses against a real temp repo, nothing is mocked. If you're
  fixing a bug, reproduce it with a failing test first if you can.
- **User-facing text**: lowercase, plain, direct, never marketing language.
  New result/error text for an existing command goes in
...
- **Frontend state**: a new feature area gets its own hook in `src/hooks/`
  (see `useStashActions.ts` or `useResetRevert.ts` for the shape most hooks
  follow) rather than growing `App.tsx` further.
