...
fn recognizes_timeouts_and_connection_failures_as_network_not_auth() {
    assert!(looks_like_network_error("ssh: connect to host example.com port 22: Operation timed out"));
...
fn hard_timeout_kills_a_hung_git_process() {
    ...
    let result = run_git_with_timeout(&path, &["fetch"], Duration::from_secs(2)).expect("run_git_with_timeout itself should not error");
    ...
    assert!(elapsed < Duration::from_secs(10), "took {elapsed:?} — timeout did not actually bound the wait");
...
fn current_branch_name_works_before_the_first_commit() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.to_string_lossy().to_string();
    Command::new("git").arg("-C").arg(&path).arg("init").arg("-q").output().unwrap();

    let branch = current_branch_name(&path).expect("should resolve even with zero commits");
    assert!(!branch.is_empty(), "branch name should not be empty before any commit exists");
...
fn init_repo_creates_a_working_repo_and_reports_a_real_branch_name() {
    ...
    let info = init_repo_sync(path.clone(), None).expect("init_repo_sync should succeed on an empty folder");
    assert_eq!(info.path, path);
    assert!(!info.current_branch.is_empty(), "should have a real branch name, not the empty string this bug used to produce");

    let check = Command::new("git").arg("-C").arg(&path).args(["rev-parse", "--is-inside-work-tree"]).output().unwrap();
...
fn init_repo_links_the_given_remote_url_as_origin() {
    ...
    init_repo_sync(path.clone(), Some("https://example.com/team/project.git".to_string()))
        .expect("init_repo_sync with a remote should succeed");

    let remotes = Command::new("git").arg("-C").arg(&path).args(["remote", "-v"]).output().unwrap();
    let remotes = String::from_utf8_lossy(&remotes.stdout);
    assert!(remotes.contains("origin") && remotes.contains("https://example.com/team/project.git"), "remotes were: {remotes}");
...
fn repo_name_from_url_handles_https_and_scp_style_urls() {
    ...
    assert_eq!(repo_name_from_url("https://github.com/user/my-repo"), "my-repo");
    assert_eq!(repo_name_from_url("https://github.com/user/my-repo/"), "my-repo");
    ...
    assert_eq!(repo_name_from_url("  https://github.com/user/spaced.git  "), "spaced");
}
...
fn clone_repo_creates_a_named_subfolder_with_real_history() {
    ...
    git(&dir.to_string_lossy(), &["init", "--bare", "-q", &remote_path]);
    ...
    let info = clone_repo_sync(remote_path.clone(), dest_path.clone()).expect("clone should succeed");
    assert_eq!(info.name, "remote");
    assert_eq!(info.path, dest_dir.join("remote").to_string_lossy().to_string());
    assert_eq!(info.current_branch, "main");
    assert!(dest_dir.join("remote").join("README.md").exists(), "cloned working tree should have the real file");

    let second = clone_repo_sync(remote_path, dest_path);
    assert!(second.is_err(), "cloning into an already-used name should fail, not overwrite");
...
fn check_git_identity_reads_the_repo_s_configured_name_and_email() {
    ...
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.to_string_lossy().to_string();
    Command::new("git").arg("-C").arg(&path).args(["init", "-q"]).output().unwrap();
    Command::new("git").arg("-C").arg(&path).args(["config", "user.name", "Ada Lovelace"]).output().unwrap();
    Command::new("git").arg("-C").arg(&path).args(["config", "user.email", "ada@example.com"]).output().unwrap();
...
fn git_config_value_returns_none_for_a_key_that_was_never_set() {
    ...
    assert_eq!(git_config_value(&path, "gitroot.definitely-not-a-real-key"), None);
